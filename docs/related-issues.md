...
参照確認日: **2026-09-08**。保護再生経路を調べるための手掛かりであり、以下の報告がDisney+ 4KHDR Enhancedの実験で発生したフリーズと同一原因だと確認したものではありません。今後、リンク先の公開範囲・修正状態が変わる可能性があります。

この文書追加に伴う新しい再生実験・ドライバー導入は行っていません。プロジェクト自身の実再生結果は[診断記録](diagnostics.md)を参照してください。

後続のノート側の記録では、[Intel内蔵GPUの表示・デコード経路で4Kを約5分14秒継続再生](intel-4k.md)できています。同ノートのNVIDIA直結経路では1080p段階で出力保護エラーがあり、4K実復号は未試行です。この差を以下の公開チケットの原因確定・修正確認とは扱いません。
